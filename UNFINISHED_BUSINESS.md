...
| 4 | **Tape 02's non-constant tick rate** (FINDINGS §25) | The 44.5-vs-46.0 reconciliation that used to sit here is **closed** — both figures were wrong, the answer is a count, 45.000, and it holds in every 2026 fragment measurable to the frame (FINDINGS §10.5). What survives is the one exception: Case 11 "Tin bird primer" on Tape 02 runs **33 to 46 frames per tick, drifting *within* the single fragment**, where everything else sits at a constant 45. Variable-speed source or a different processing path for that tape — untested either way | 1–2 h |
